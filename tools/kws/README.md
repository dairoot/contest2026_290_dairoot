...
- **v7g（板上当前版本，`models/nihao_openvela_v7/`）= v7b + 作者本人
  44 条板麦唤醒词（30 训 / 14 留出，每条 24 窗）+ 本人 49 句非唤醒语音 /
  单说的 openvela / 敲击声 + 260 个本人房间底噪窗（负例，8×/3×）+ 变速
  上限 1.35×**。本人留出 14 条召回 71%@0.85（v7e 64%、v7b 38%），本人
  非唤醒语音误触 0/49、房间底噪窗 0/260（v7e 各 2%——v7e 只有正例带着
  本人底噪签名，说完一遍后窗口只剩底噪也停在 0.85 以上，每 2 s 不应期一过
  就再触发）；real-neg 0.3%、tts-neg 0%。代价：合成/重录语料召回明显降
  （rerec-pos 101 条 63%、tts-pos 91%），1.4 万参数正被真人数据拉偏。
  剩下的弱例是极快（整句 0.8–1.0 s）和轻声（-47 dBFS）的说法，多录几批、
  或换个人再录，`cut_takes.py` 十分钟一轮。
